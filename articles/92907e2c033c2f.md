...
**参考にした[ガイド](https://steamcommunity.com/sharedfiles/filedetails/?id=3465376526)でこの手順を説明していますが、これを実行するとGodotでゲームが起動(デバッグ)出来なくなります。**
**これはSteamサーバへの接続/認証に関わる部分なので、特に対人やランキング要素があるゲームではこの手順によりSteamサーバへの接続を断っておいた方が良いように思います。**
...
デコンパイルが完全かつ`Godot Steam`プラグインが正規の状態の場合はGodotからテスト出来ます。
パッキングなどは不要で、`F5キー`または画面右上の`プロジェクトを実行`からゲームを実行します。
必要に応じてブレークポイントを貼ります。

Godotからテスト出来ない場合はゲームにMODを組み込んでテストします。
